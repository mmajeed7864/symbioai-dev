...
test("speech v2 accepts only an exact generated coach-reply text envelope", () => {
  const parsed = parseSpeechRequest(rawRequest());
  assert.equal(parsed.ok, true);
  assert.equal(parsed.request.text, rawRequest().text);
  assert.equal(FITCOACH_SPEECH_VERSION, "2026-08-31.2");

  assert.equal(
    parseSpeechRequest({ ...rawRequest(), audio: "must-not-exist" }).error,
  ...
  for (const classification of [
    ...
    const result = parseSpeechRequest(rawRequest({ data_classification: classification }));
    assert.equal(result.status, 400, classification);
    assert.equal(result.error, "UNSUPPORTED_DATA_CLASSIFICATION", classification);
  }
  assert.equal(
    parseSpeechRequest(rawRequest({ voice_gender: "unknown" })).error,
    "INVALID_REQUEST_CONFIGURATION"
  );
  assert.equal(
    parseSpeechRequest(rawRequest({ voice_profile: "unknown" })).error,
    "INVALID_REQUEST_CONFIGURATION"
  );
  assert.equal(
    parseSpeechRequest(rawRequest({ voice_gender: "female", voice_profile: "atlas" })).error,
    "INVALID_REQUEST_CONFIGURATION"
  );
  assert.equal(
    parseSpeechRequest(rawRequest({ tone: "cruel" })).error,
    "INVALID_REQUEST_CONFIGURATION"
  );
  assert.equal(
    parseSpeechRequest(rawRequest({ text: "x".repeat(1_201) })).error,
    "INVALID_REQUEST_CONFIGURATION"
  );
});
...
test("female and male profiles have reviewed tone-specific delivery settings", () => {
  ...
  const strict = resolveVoiceProfile(
    parsedRequest({ tone: "strict", voice_gender: "male", voice_profile: "atlas" }),
    {
      FITCOACH_ELEVENLABS_FEMALE_VOICE_ID: "femaleBaseVoice12345",
  ...
  const directBritish = resolveVoiceProfile(
    parsedRequest({ tone: "direct", voice_gender: "male", voice_profile: "bennett" }),
    {
      FITCOACH_ELEVENLABS_MALE_VOICE_ID: "maleBaseVoice123456",
  ...
  assert.ok(
    resolveVoiceProfile(
      parsedRequest({ tone: "competitive", voice_gender: "male", voice_profile: "atlas" }),
      {}
    ).voiceSettings.style > strict.voiceSettings.style
  );
  assert.equal(supportive.modelId, "eleven_flash_v2_5");
  const builtInBritish = resolveVoiceProfile(
    parsedRequest({ tone: "rude", voice_gender: "male", voice_profile: "bennett" }),
    {}
  );
...
test("speech character budget is monthly, bounded, and releases rejected overage", async () => {
  ...
  const first = await reserveSpeechCharBudget(redis, {
    chars: 4_000,
    at,
    monthlyCharBudget: 10_000,
  });
  const second = await reserveSpeechCharBudget(redis, {
    chars: 7_000,
    at,
    monthlyCharBudget: 10_000,
  });
