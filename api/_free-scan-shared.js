...
export async function upsertFreeScan(payload, req) {
...
export async function listFreeScans(limit = 250) {
  const { url, key, table } = requireSupabaseEnv();
...
export function verifySyncToken(req) {
...
async function sendTelegram(payload) {
  const botToken =
    process.env.TELEGRAM_BOT_TOKEN || process.env.OLYMPUS_TELEGRAM_BOT_TOKEN || "";
  const chatId =
    process.env.TELEGRAM_CHAT_ID || process.env.OLYMPUS_TELEGRAM_MOHAMMED_CHAT_ID || "";
  if (!botToken || !chatId) {
    return { configured: false, ok: false, detail: "Telegram is not configured." };
  }
  const text = [
    "P0 FREE SCAN - reply first",
    "",
    `Business: ${payload.business || "not provided"}`,
    `Name: ${payload.name || "not provided"}`,
    `Email: ${payload.email || "not provided"}`,
...
export async function sendNotifications(payload) {
  const events = [];
  try {
    events.push({ channel: "telegram", ...(await sendTelegram(payload)) });
  } catch (error) {
    events.push({ channel: "telegram", configured: true, ok: false, error: error.message });
  }
  try {
